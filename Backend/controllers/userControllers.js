      ...
      if (isEmpty(user)) {
        bcrypt.genSalt(10, async (err, salt) => {
          try {
            ...
            return res
              .status(500)
              .send("Error generating hash or creating user");
          }
        });
      } else {
  ...
  if (!isValid) {
    ...
    await User.findOne({ email: email })
      ...
      .catch((error) => {
        res
          .status(404)
          .send({ isAutheticate: false, msg: "User cannot found" });
      });
  }
};
...
const updateUser = async (req, res) => {
  if (req.user.id === req.params.id || req.user.userType === "admin") {
    ...
    return res
      .status(401)
      .send("You are not autherized to update this account");
  }
};
...
const resetPwd = async (req, res) => {
  ...
  if (!isEmpty(email) && !isEmpty(password) && !isEmpty(confirmationCode)) {
...
const getUserById = async (req, res) => {
  try {
    const user = await User.findById(req.params.id); // Assuming req.params.id contains the user ID
    if (!user) {
