...
const ViewFacility = () => {
  ...
  const handleInput = (event) => {
    ...
    if (event.target.name === "facName") setFacName(event.target.value);
    if (event.target.name === "description") setFacDesc(event.target.value);

    if (event.target.name === "cost") setFacCost(event.target.value);

    if (event.target.name === "location") setFacLocation(event.target.value);

    if (event.target.name === "capacity") setFacCapacity(event.target.value);

    if (event.target.name === "address") setFacAddress(event.target.value);

    if (event.target.name === "rules") setFacRules(event.target.value);
  };
  ...
  const hanldeSubmit = async (event) => {
    ...
    let facilityId = "";
    await axios
      .post("http://localhost:4000/api/facility/register", formData, {
        headers: {
          Authorization: token,
        },
      })
  ...
  const uploadImages = async (facilityId, imageFiles) => {
    ...
    await axios
      .post(
        `http://localhost:4000/api/facility/uploadPhotos/${facilityId}`,
        formData,
        {
          headers: {
        ...
        <div className={viewFacility_css.contentImage}>
          <div className={viewFacility_css.body}>
              ...
              {options.map((facility, index) => {
                ...
                return (
                  <Grid item xs={6} key={index}>
                    <Card
                      sx={{ maxWidth: 550, maxHeight: 400 }}
                      className={viewFacility_css.card}
                    >
                      <Link
                        to={`/facility/${facility._id}`}
                        className={viewFacility_css.cardLink}
                      >
                        <CardActionArea>
                          <CardMedia
            ...
            <Dialog
              open={open}
              onClose={handleClose}
              className={viewFacility_css.dialogBox}
            >
